...
def spatial_map_single_plot_LIS_diff(land_ctl_files, land_sen_files, var_names, time_s=None,
    ...
    for var_name in var_names:
        ...
        if var_name in ["Tair_f_inst",]:
            # Dec1       = spital_var_max(time,Ctl_tmp,time_s,time_e)
            # Clim       = spital_var_max(time,Sen_tmp,time_s,time_e)
            Dec1       = spital_var(time,Ctl_tmp,time_s,time_e)
            Clim       = spital_var(time,Sen_tmp,time_s,time_e)
        else:
            Dec1       = spital_var(time,Ctl_tmp,time_s,time_e)
            Clim       = spital_var(time,Sen_tmp,time_s,time_e)
        ...
        if var_name in ['ESoil_tavg','Evap_tavg',"ECanop_tavg",'TVeg_tavg',"Rainf_tavg","Snowf_tavg","Qs_tavg","Qsb_tavg"]:
            Dec1     = Dec1*3600*24*365
            Clim     = Clim*3600*24*365
...
if __name__ == "__main__":
    ...
    if 0:
        ...
        case_ctl  = "drght_2017_2019_"+case_name+"_climatology"
