...
def output_spatial_FMI(atmo_ctl_path, atmo_sen_path, land_ctl_path, land_sen_path, FMI_out, time_s=None, time_e=None,
    ...
    T_Ctl_daily    = time_clip_to_day(time_T, T_Ctl,   time_s, time_e)
    T_Sen_daily    = time_clip_to_day(time_T, T_Sen,   time_s, time_e)
    RH_Ctl_daily   = time_clip_to_day(time_RH, RH_Ctl, time_s, time_e)
    RH_Sen_daily   = time_clip_to_day(time_RH, RH_Sen, time_s, time_e)
    ...
    f.history           = "Created by: %s" % (os.path.basename(__file__))
    f.creation_date     = "%s" % (datetime.now())
    f.description       = 'FFDI for ctl and sen simulations for 201909-202002, made by MU Mengyuan'
