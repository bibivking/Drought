...
def get_time_cood(file_path, file_name, time_s ,time_e):

    # ==================== process time ====================
    # Read in time steps
    ncfile      = Dataset(file_path+"z/"+file_name)
...
def plot_profile_wrf_wind(atmo_path_ctl, atmo_path_sen, wrf_path, land_path, file_name,
    ...
    time_cood_all, time_cood_day, time_cood_night, doy_all, doy_day, doy_night = \
                                        get_time_cood(atmo_path_ctl, file_name, time_s, time_e)
    ...
    ax[1,1].quiverkey(q,X=0.80, Y=2.1, U=0.2, label=str(scale)+' m/s', labelpos='E', color="black")
